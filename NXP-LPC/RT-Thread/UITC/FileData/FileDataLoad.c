...
static INT32U LoadGatageCfg(INT8U *pBuf,SEC_GATAGE_CFG *pCfg)
{
	...
	memcpy((void *) & pCfg->MaxGatage, &pBuf[nBufPos],sizeof(FP64)); 
	nBufPos += sizeof(FP64);

	memcpy((void *) & pCfg->MinGatage, &pBuf[nBufPos],sizeof(FP64)); 
	nBufPos += sizeof(FP64);

	memcpy((void *) & pCfg->MaxEncoderVal, &pBuf[nBufPos],sizeof(INT32U)); 
	nBufPos += sizeof(INT32U);

	memcpy((void *) & pCfg->MinEncoderVal, &pBuf[nBufPos],sizeof(INT32U)); 
	nBufPos += sizeof(INT32U);
	...
	memcpy((void *) & pCfg->ParaCnt, &pBuf[nBufPos],sizeof(INT32U)); 
	nBufPos += sizeof(INT32U);
