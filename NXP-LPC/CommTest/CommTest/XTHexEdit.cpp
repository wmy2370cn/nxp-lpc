...
CXTHexEdit::CXTHexEdit()
{
	...
	m_bShowCaret        = TRUE;
	m_bUpdate           = TRUE;
	m_bAllowDeletes     = false;
	m_ptEditPos.x       = 0;
	m_ptEditPos.y       = 0;
	m_eEditMode         = editNone;
	m_szCaret           = CSize(0, 0);
//	m_crBack            = xtAfxData.clrWindow;//GetXtremeColor(COLOR_WINDOW);
	m_crBack            = RGB(255,255,255);//GetXtremeColor(COLOR_WINDOW);
	m_crText            = xtAfxData.clrWindowText;// GetXtremeColor(COLOR_WINDOWTEXT);
	m_crDisabledBack    = xtAfxData.clr3DFace;//GetXtremeColor(COLOR_3DFACE);;
	m_crDisabledText    = xtAfxData.clrGrayText;//  GetXtremeColor(COLOR_GRAYTEXT);
	m_crHighlightBack   = xtAfxData.clrHighlight;//GetXtremeColor(COLOR_HIGHLIGHT);
	m_crHighlightText   = xtAfxData.clrHighlightText;//GetXtremeColor(COLOR_HIGHLIGHTTEXT);
	m_crDisabledHighlightText = xtAfxData.clrHighlightText;//GetXtremeColor(COLOR_HIGHLIGHTTEXT);
...
void CXTHexEdit::OnPaint()
{
	...
	if(m_pData)
	{
		...
		if(m_bShowAddress)
		{
			...
			// draw address
			int i;
			for (i = m_nTopIndex; (i < (m_nLength + 1)) && (rcd.TopLeft().y < height); i+= m_nBytePerRow)
			{
				int width = m_bAddressIsWide ? 8 : 4;
				SPRINTF_S(buf, _countof(buf), _T("%0*lX"), width, m_dwBaseAddress + i);
			//	width += 2;
				memDC.DrawText(buf, width, rcd,  DT_LEFT|DT_TOP|DT_SINGLELINE|DT_NOPREFIX);
...
void CXTHexEdit::OnEditCopy()
{
	...
	{
		hMema = ::GlobalAlloc(GMEM_MOVEABLE|GMEM_DDESHARE|GMEM_ZEROINIT,dwLen * 3);
...
void CXTHexEdit::SelDelete(int nSelStart, int nSelEnd)
{
	...
	MEMMOVE_S(m_pData + nSelStart, m_pData + nSelEnd,m_nLength - nSelEnd);
