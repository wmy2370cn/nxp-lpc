...
protected:
	...
	struct pie_3d_properties
	{
		...
		Gdiplus::PointF pt_End;			//The end location point for a single element on upper face of pie
	};
private:
...
//The color lable properties, these are relatively resized.
	...
	struct	label_rect_info{
		...
		Gdiplus::Color lbTextColor;
		Gdiplus::Color lbValueRectColor;
...
private: //internal functions//
	...
	void DrawLabels(Gdiplus::Graphics* graphics);	//Draw the color and % value labels
	...
	Gdiplus::Color CalculateGradientLight(Gdiplus::Color crBase, float fGrad);	//function to calculate the gradient light color for a given color, gradient value
	Gdiplus::Color CalculateGradientDark(Gdiplus::Color crBase, float fGrad);		//function to calculate the gradient dark color for a given color, gradient value
	...
	void DrawPiechartPecentages(Gdiplus::Graphics* pGraphics, float flStartAngle);	//Drawing function for the % values on pie chart
	BOOL ReArrangeElementMap(void);	//Function used to rearrange the element std::map in removing items.
	void DrawBackGround(Gdiplus::Graphics* pGraphics);	//Drawing function for the background area
	...
	float CacluateInclineAngle(Gdiplus::PointF ptElement, CRect rectTop);
	BOOL HasElement(CString sElement);	//Check the element exist by its element name ID
	BOOL SaveImageInternal(Gdiplus::Bitmap* btImage);
	...
	BOOL ConstructRectangularFace(Gdiplus::Graphics* pGraphics, pie_chart_element pElement, Gdiplus::PointF ptTopCenter, Gdiplus::PointF ptClicked); //Sub 3-D face drawing function
	BOOL ConstructArcFace(Gdiplus::Graphics* pGraphics, pie_chart_element* pElement, Gdiplus::REAL flStart, Gdiplus::RectF rect,  Gdiplus::PointF ptClicked);	//Sub 3-D face drawing function
	BOOL ConstructSideFace(Gdiplus::Graphics* pGraphics, pie_chart_element pElement,Gdiplus::REAL flDisplace,												//Sub 3-D face drawing function
								Gdiplus::REAL flStart, Gdiplus::PointF ptClicked);
	
	BOOL Construct3DElement(Gdiplus::Graphics* pGraphics, pie_chart_element* pElement,	//The mainfunction to draw the 3-D shape elements
								  Gdiplus::REAL flStart, pie_drawn_direction drawCase, Gdiplus::PointF ptClicked);
	BOOL Construct3DElementSpecific(Gdiplus::Graphics* pGraphics, pie_chart_element* pElement,	//Draw special cases of 3-D elements when the front element = last element
								  Gdiplus::REAL flStart, pie_drawn_direction drawCase, Gdiplus::PointF ptClicked);
	
	//+ Function to find the 3-D element clicked
	BOOL FindClicked3DElement(Gdiplus::PointF ptClicked);

public: //External functions//
	//Insert item function
	PIECHARTITEM InsertItem(CString sElement, CString sLabel, double dValue, Gdiplus::Color crColor);
	...
	BOOL UpdateItemColor(PIECHARTITEM pItem, Gdiplus::Color crItem);

	//Font set functions
	void SetLabelFont(LOGFONT lf);
	void SetTitleFont(LOGFONT sFontName);

	//Gdiplus::Color set functions
	void SetLabelBoxColor(Gdiplus::Color crTextColor);
	void SetBackgrndColor(Gdiplus::Color crBkgColor);
	void SetLabelColor(Gdiplus::Color crLbColor);
	void SetItemHighlightColor(Gdiplus::Color crHighlightColor);
