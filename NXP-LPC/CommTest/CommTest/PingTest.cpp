...
BOOL  CPingTask::PingTest( )
{
	...
	if (IsSocketReadible(m_hSocketRaw, m_dwTimeout, bReadable))
	{
		...
		m_PingReply.nError = DecodeResponse(m_pRecvBuf, nRead, &from);
...
int CPingTask::DecodeResponse(unsigned char* pBuf, int nBytes, sockaddr_in* from) 
{
	...
	if (pIcmpHdr->i_id != (USHORT)m_nThreadID) 
	{
...
UINT  PingTestTask (LPVOID lpParam)
{
	...
	szLog.Format(_T("PING %d.%d.%d.%d 线程[%d]启动。"),dst_addr.sin_addr.s_net ,dst_addr.sin_addr.s_host ,
		dst_addr.sin_addr.s_lh ,dst_addr.sin_addr.s_impno,pTask->m_nThreadID );
...
void CPingTask::StartTask()
{
	...
	if (m_pPingTsk == NULL)
	{
		...
		m_nThreadID = m_pPingTsk->m_nThreadID;
