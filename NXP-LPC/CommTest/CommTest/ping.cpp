...
BOOL CPing::DecodeResponse(LPIP_HEADER pIPHeader, int nBytes, sockaddr_in* from, USHORT nExpectedSequenceNumber, USHORT nID,BOOL& bTryAgain) const
{
	//To get rid of unreferenced formal parameter warning
	from;
...
BOOL CPing::PingUsingWinsock(DWORD dwAddr, CPingReply& pr, UCHAR nTTL, DWORD dwTimeout, WORD wDataSize, USHORT nSequenceNumber, USHORT nIdentifier, UCHAR nTOS, BOOL bDontFragment, LPCTSTR pszLocalBoundAddress) const
{
	//Use the High performace counter to get an accurate RTT
	LARGE_INTEGER Frequency;
	Frequency.QuadPart = 0;
	...
	while (bContinueReading)
	{
		...
		if (IsSocketReadible(sockRaw, dwTimeout, bReadible))
		{
			if (bReadible)
			{
				...
				{
					//Decode the response we got back
					bSuccess = DecodeResponse(pRecvBuf, nRead, &from, nSequenceNumber, nIdentifier,bContinueReading);
				}
				else
				{
					DWORD dwError = GetLastError();
