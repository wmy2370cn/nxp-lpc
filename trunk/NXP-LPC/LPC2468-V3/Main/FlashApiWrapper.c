...
#else  //2292
#define  FCB_BLOCK_SIZE  8*1024  //0X0A
#define  BIN_BLOCK_SIZE  32*1024 //0X0b-0X0e 4个8K
#define  SRC_BLOCK_SIZE  64*1024 //0X09 1个64K
#endif // LPC_2468
...
#else
#define	 FCB_BLOCK_DESTADDR 	0x00030000      //扇区0X0a
#define  BIN_BLOCK_DESTADDR 	0x00032000		//0X0b-0X0e 4个8K
#define  SRC_BLOCK_DESTADDR 	0x00020000		//0X09 1个64K
#endif
...
extern INT8U SaveData( INT8U *err )
{
	...
	//先检查一下看看BIN需要几个分区
	nSector = (INT8U)(pTmpFcb->nBinLen / SECTOR_SIZE);

	OS_ENTER_CRITICAL();	
	//2.0 FCB
	SelSector(0x0a, 0x0a);					// 选择扇区
	EraseSector(0x0a, 0x0a);
	SelSector(0x0a, 0x0a);					// 选择扇区
	RamToFlash(FCB_BLOCK_DESTADDR, (INT32U)FCB_BLOCK, FCB_BLOCK_SIZE);	// 写数据区到FLASH	
	...
	for (i = 0; i < nSector; i++)
	{
		SelSector(0x0B+i, 0x0B+i);					// 选择扇区
		EraseSector(0x0B+i, 0x0B+i);
		SelSector(0x0B+i, 0x0B+i);					// 选择扇区
		RamToFlash(BIN_BLOCK_DESTADDR, (INT32U) &BIN_BLOCK[i*SECTOR_SIZE], SECTOR_SIZE);	// 写数据区到FLASH	
	}

	if (pTmpFcb->nCfgFileLen)
	{
		//2.2 SRC 
		SelSector(0X09, 0X09);					// 选择扇区
		EraseSector(0X09, 0X09);
		SelSector(0X09, 0X09);					// 选择扇区
		RamToFlash(SRC_BLOCK_DESTADDR, (INT32U)SRC_BLOCK, SRC_BLOCK_SIZE);	// 写数据区到FLASH		 
	}
