...
**------------------------------------------------------------------------------------------------------
********************************************************************************************************/
static INT8U GetOneFrame(CMBExt * pMbx, INT8U c)
{
	INT16U chkcrc = 0;
	...
	{	
		...
		if(pMbx->RxCount==1)
		{
			...
			else if (c == MB_ADDRESS_BROADCAST )
			{
				pMbx->Broadcast = TRUE;
				pMbx->ModbusRxCRC=0xFFFF;
				crc16_ex(&c,1,& pMbx->ModbusRxCRC);
...
**------------------------------------------------------------------------------------------------------
********************************************************************************************************/
static INT8U RecvData( CMBExt * pMbx )
{
	INT8U  Err = 0;
	INT16U nRecvNum = 0;
	INT16U i = 0;
...
**------------------------------------------------------------------------------------------------------
********************************************************************************************************/
static INT8U PasreMsg(CMBExt * pMbx)
{
...
static void SendResponse(CMBExt *pMbx)
{
	INT32U temp = 0;
	...
	//	temp = WriteUart (pMbx->UartId, &pMbx->TxBuff[pMbx->TxCount], pMbx->TxLen-pMbx->TxCount,  NULL);
		temp = DirectWriteUartApi(pMbx->UartId, &pMbx->TxBuff[pMbx->TxCount], pMbx->TxLen-pMbx->TxCount);
