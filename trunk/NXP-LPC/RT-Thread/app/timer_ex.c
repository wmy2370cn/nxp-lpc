...
/* #define TIMER_DEBUG */
static rt_list_t rt_timer_ex_list;


static struct rt_thread timer_ex;
static rt_uint8_t rt_thread_stack[TIMER_THREAD_STACK_SIZE];
 ...
 */
void rt_timer_ex_timeout_sethook(void (*hook)(struct rt_timer* timer))
{
...
void rt_timer_ex_check()
{
	...
	for (n = rt_timer_ex_list.next; n != &(rt_timer_ex_list); )
	{
...
static void rt_thread_timer_ex_entry(void* parameter)
{
   (void)parameter;       
	while (1)
	{
		rt_sem_take(&timer_sem,0);
 ...
 *
 */
void rt_system_timer_ex_init()
{
	rt_list_init(&rt_timer_ex_list);
	rt_sem_init(&timer_sem, "timer", 1, RT_IPC_FLAG_FIFO);
	timer_ex_cnt = 0;
	/*init timer_ex thread*/
 	rt_thread_init(&timer_ex,
		"timer",
		rt_thread_timer_ex_entry, RT_NULL,
		&rt_thread_stack[0], sizeof(rt_thread_stack),
		TIMER_EX_THREAD_PRIO, 10);

	/* startup */
	rt_thread_startup(&timer_ex);
}

static void _rt_timer_ex_init(rt_timer_t timer, 
 ...
 */
rt_timer_t rt_timer_ex_create(const char* name, void (*timeout)(void* parameter), void* parameter, rt_tick_t time, rt_uint8_t flag)
{
...
rt_err_t rt_timer_ex_start(rt_timer_t timer)
{
	...
	for (n = rt_timer_ex_list.next; n != &rt_timer_ex_list; n = n->next)
	{
		t = rt_list_entry(n, struct rt_timer, list);
